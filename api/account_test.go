...
import (
	...
	mockdb "github.com/xdot2012/simple-bank/db/mock"
...
func TestListAccountsAPI(t *testing.T) {
	...
	n := 5
	for i := 0; i < n; i++ {
		accounts[i] = randomAccount(t, user.ID)
	}
...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			accountID: account.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, invalidUser.ID, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account.ID)).
...
func randomAccount(t *testing.T, ownerID int64) db.Account {
	return db.Account{
